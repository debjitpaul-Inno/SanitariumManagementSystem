...
class BedListController extends Controller
{
        ...
        try {
            if ($request->ajax()) {
                ...
                return DataTables::of($data)->editColumn('description',  function(BedList $bedList) {
                    ...
                    ->addColumn('bed_type_id',  function($row) {
                        if ($row->bedTypes != null){
                            return '<a href="'.route('bed-type.show',$row->bedTypes->uuid ?? '').'" class="badge-link-success" style="text-decoration: none">'. $row->bedTypes->title  . '</a>' ?? 'N/A' ;
                        }else{
                    ...
                    ->rawColumns(['action','status','availability','bed_type_id'])
                    ->make(true);
            }
    ...
    public function create()
    {
        ...
        return view('bed.bedList.create', ['bedTypes' => $bedtypes,'rooms'=>$rooms]);
    }
    ...
    public function edit($uuid)
    {
        ...
        $rooms = Room::all();
        return view('bed.bedList.edit', ['bedList' => $bedList, 'bedTypes' => $bedTypes,'rooms'=>$rooms]);
    }
